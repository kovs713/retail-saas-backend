...
export class RegistrationApplicationController {
  ...
  @ApiResponse({
    ...
    type: [RegistrationApplicationDto],
  })
  async list(
    @Query('status')
    status: RegistrationStatus,
  ): Promise<AppApiResponse<RegistrationApplicationDto[]>> {
    const applications = await this.registrationApplicationService.list(status);
    return {
      success: true,
      data: RegistrationApplicationDto.fromEntities(applications),
    };
  }
  ...
  @Post(':id/reject')
  @ApiOperation({
    summary: 'Reject registration application',
  })
  @ApiParam({
    name: 'id',
    type: String,
  ...
  async reject(
    ...
    id: string,
    @Body()
    body: RejectRegistrationApplicationDto,
  ): Promise<AppApiResponse<RegistrationApplicationDto>> {
    const application = await this.registrationApplicationService.reject(
      id,
      body.reason,
    );
    return {
