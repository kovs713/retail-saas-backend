...
import { CreateLocationDto, CreateShopDto, LocationDto, ShopDto, UpdateLocationDto, UpdateShopDto } from './dto';
...
export class ShopController {
  ...
  @UseGuards(AuthGuard, RolesGuard)
  @Roles(Role.OWNER, Role.ADMIN)
  @ApiOperation({ summary: 'Create a location for a shop (Owner or Admin)' })
  @ApiBearerAuth('JWT')
  @ApiParam({ name: 'shopId', type: String, description: 'Shop ID' })
  @ApiResponse({ status: 201, description: 'Location created successfully', type: LocationDto })
  @ApiResponse({ status: 400, description: 'Bad request - Maximum 3 active locations reached' })
  @ApiResponse({ status: 404, description: 'Shop not found' })
  async createLocation(
    ...
    const location = await this.shopService.createLocation(shopId, createLocationDto);
    return { success: true, data: LocationDto.fromEntity(location), message: 'Location created successfully' };
  }

  @Get(':shopId/locations')
  @UseGuards(AuthGuard, RolesGuard)
  @Roles(Role.OWNER, Role.ADMIN)
  @ApiOperation({ summary: 'List locations for a shop (Owner or Admin)' })
  @ApiBearerAuth('JWT')
  @ApiParam({ name: 'shopId', type: String, description: 'Shop ID' })
  @ApiResponse({ status: 200, description: 'Locations retrieved successfully', type: [LocationDto] })
  @ApiResponse({ status: 404, description: 'Shop not found' })
  async findLocations(
    @Param('shopId') shopId: string,
    @Tenant() tenantContext: TenantContext,
  ...
  @UseGuards(AuthGuard, RolesGuard)
  @Roles(Role.OWNER, Role.ADMIN)
  @ApiOperation({ summary: 'Get a location by ID (Owner or Admin)' })
  @ApiBearerAuth('JWT')
  @ApiParam({ name: 'shopId', type: String, description: 'Shop ID' })
  @ApiParam({ name: 'id', type: String, description: 'Location ID' })
  @ApiResponse({ status: 200, description: 'Location retrieved successfully', type: LocationDto })
  ...
  @ApiParam({ name: 'shopId', type: String, description: 'Shop ID' })
  @ApiParam({ name: 'id', type: String, description: 'Location ID' })
  @ApiResponse({ status: 200, description: 'Location updated successfully', type: LocationDto })
  @ApiResponse({ status: 404, description: 'Location not found' })
  async updateLocation(
    ...
    this.assertShopAccess(shopId, tenantContext, req);
    const location = await this.shopService.updateLocation(shopId, id, updateLocationDto);
    return { success: true, data: LocationDto.fromEntity(location), message: 'Location updated successfully' };
  }
