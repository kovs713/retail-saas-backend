...
export class AdminOrderController {
  ...
  @ApiResponse({ status: 200, description: 'Orders list', type: OrderListResponseDto })
  async getOrders(
    ...
    const result = await this.orderService.findByShopId(tenantContext.shopId, { page, limit, status });

    return {
      success: true,
      data: {
        data: result.data.map((order) => this.orderService.toResponseDto(order)),
        total: result.total,
        page: result.page,
        limit: result.limit,
      },
  ...
  async updateOrderStatus(
    ...
    const order = await this.orderService.updateStatus(id, tenantContext.shopId, updateStatusDto);
    const response = this.orderService.toResponseDto(order);

    return { success: true, data: response, message: 'Order status updated successfully' };
  }
}
