...
export class AnalyticsService {
  ...
  async getRevenueWithPercent(
    shopId: string,
  ): Promise<{ current: number; percentFromLastMonth: number }> {
    const now = new Date();
  ...
  async getShopGrowth(shopId: string): Promise<number> {
    const now = new Date();
    const firstDayOfMonth = new Date(now.getFullYear(), now.getMonth(), 1);
    const firstDayOfLastMonth = new Date(
      now.getFullYear(),
    ...
    const [revenueCurrent, revenueLast, ordersCurrent, ordersLast] =
      await Promise.all([
        this.orderRepository.getTotalRevenue(shopId, firstDayOfMonth, now),
        this.orderRepository.getTotalRevenue(
          shopId,
