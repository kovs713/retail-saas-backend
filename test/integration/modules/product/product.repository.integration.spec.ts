...
describe('ProductRepository Integration', () => {
  ...
  const createShop = async (name: string, slug: string, isActive: boolean = true): Promise<Shop> => {
  ...
  const createProduct = async (overrides: Partial<Product> & Pick<Product, 'shopId' | 'sku' | 'name' | 'price'>) => {
  ...
  it('findAll returns only non-deleted products for the target shop ordered by newest first', async () => {
    ...
    const firstProduct = await createProduct({ shopId: shopA.id, sku: 'SKU-001', name: 'First', price: 100 });
    const secondProduct = await createProduct({ shopId: shopA.id, sku: 'SKU-002', name: 'Second', price: 200 });
    const deletedProduct = await createProduct({ shopId: shopA.id, sku: 'SKU-003', name: 'Deleted', price: 300 });
    await createProduct({ shopId: shopB.id, sku: 'SKU-004', name: 'Other shop', price: 400 });
    await repository.softDeleteById(deletedProduct.id);

    const [products, total] = await repository.findAll(shopA.id, {});

    expect(total).toBe(2);
    expect(products.map((product) => product.id)).toEqual([secondProduct.id, firstProduct.id]);
  });
  ...
  it('findAll applies category, search, price range and sorting behavior', async () => {
    ...
    await createProduct({ shopId: shop.id, sku: 'SKU-TV', name: 'Television', price: 700, categoryId: category.id });
    ...
    await createProduct({ shopId: shop.id, sku: 'SKU-BOOK', name: 'Book', price: 40 });
  ...
  it('findById, findBySku and findByBarcode stay scoped to shop and exclude soft-deleted rows', async () => {
    ...
    await expect(repository.findById(product.id, shopA.id)).resolves.toMatchObject({ id: product.id });
    await expect(repository.findBySku('SKU-LOOKUP', shopA.id)).resolves.toMatchObject({ id: product.id });
    await expect(repository.findByBarcode('BARCODE-LOOKUP', shopA.id)).resolves.toMatchObject({ id: product.id });
    ...
    await expect(repository.findBySku('SKU-LOOKUP', shopA.id)).resolves.toBeNull();
    await expect(repository.findByBarcode('BARCODE-LOOKUP', shopA.id)).resolves.toBeNull();
  });

  it('findByIdWithShop and findByIdAndShopSlug load only products from active matching shops', async () => {
    const activeShop = await createShop('Active Shop', `active-shop-${Date.now()}`, true);
    const inactiveShop = await createShop('Inactive Shop', `inactive-shop-${Date.now()}`, false);
    const activeProduct = await createProduct({ shopId: activeShop.id, sku: 'SKU-ACTIVE', name: 'Active', price: 100 });
    ...
    const withShop = await repository.findByIdWithShop(activeProduct.id, activeShop.id);
    const byActiveSlug = await repository.findByIdAndShopSlug(activeProduct.id, activeShop.slug);
    const byInactiveSlug = await repository.findByIdAndShopSlug(inactiveProduct.id, inactiveShop.slug);
  ...
  it('findLowStock uses threshold and excludes deleted products', async () => {
    ...
    expect(defaultLowStock.map((product) => product.id)).toContain(lowStockProduct.id);
    expect(defaultLowStock.map((product) => product.id)).not.toContain(deletedLowStock.id);
    expect(defaultLowStock.map((product) => product.id)).not.toContain(highStockProduct.id);
    expect(thresholdLowStock.map((product) => product.id)).toEqual([lowStockProduct.id]);
  });
  ...
  it('countByShop and countByCategory count only non-deleted products in scope', async () => {
    ...
    const category = await createCategory(shop.id, 'Count Category', `count-category-${Date.now()}`);
    await createProduct({ shopId: shop.id, sku: 'SKU-COUNT-1', name: 'Count 1', price: 10, categoryId: category.id });
    ...
    await createProduct({ shopId: shop.id, sku: 'SKU-COUNT-3', name: 'Other category', price: 30 });
    await repository.softDeleteById(deletedProduct.id);

    await expect(repository.countByShop(shop.id)).resolves.toBe(2);
    await expect(repository.countByShop(shop.id, { categoryId: category.id })).resolves.toBe(1);
    await expect(repository.countByCategory(shop.id, category.id)).resolves.toBe(1);
  });
  ...
  it('findOneWithDeleted, updateQuantity, incrementQuantity, existsBySkuAndShop and restoreById expose persisted state changes', async () => {
    ...
    await expect(repository.existsBySkuAndShop('SKU-MUTATE', shop.id)).resolves.toBe(true);
    await expect(repository.existsBySkuAndShop('MISSING', shop.id)).resolves.toBe(false);

    await repository.updateQuantity(product.id, shop.id, 20);
    await repository.incrementQuantity(product.id, shop.id, 3);
    await repository.softDeleteById(product.id);

    const deletedProduct = await repository.findOneWithDeleted(product.id, shop.id);
